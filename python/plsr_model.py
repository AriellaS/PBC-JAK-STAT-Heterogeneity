...
class PLS_Model():
        ...
        def plot_vip(self,c="C0"):
                VIP = self.vip()
                x = np.arange(len(VIP))

                plt.bar(x,VIP, tick_label=self._labels,color=c)
                plt.title('VIP scores')
                plt.show()

        def plot_weights(self,c="k",ax=plt):
                x = np.arange(len(self._Wstar))
                VIP = self.vip()
                lightened = lighten_color(c)
                colors = list(map(lambda vip: c if vip >= 1 else lightened, VIP))
...
def lighten_color(c):
    ...
    lightened = colorsys.hls_to_rgb(hls[0], 1 - 0.3 * (1 - hls[1]), hls[2])
