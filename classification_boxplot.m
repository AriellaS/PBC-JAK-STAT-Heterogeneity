				...
				if (length(mins) > 0 & length(peaks) > 0) % shape 1 must have atleast one min and one max
					...
					if (heightdiff > 0) % max heightdiff will be 0 only if no min after peak
						...
						if (recover_slope > 0) % shape 1 must have positive slope btwn min and end
							features(i,1,factor,initval,res) = peak;
							features(i,2,factor,initval,res) = minimum;
							features(i,3,factor,initval,res) = peak_loc;
							features(i,4,factor,initval,res) = min_loc;
							features(i,5,factor,initval,res) = slope;
							features(i,6,factor,initval,res) = recover_slope;

							shape1_indicies(i,factor,initval,res) = 1;
						else
							shape1_indicies(i,factor,initval,res) = 0;
						end
					else
						shape1_indicies(i,factor,initval,res) = 0;
					end
